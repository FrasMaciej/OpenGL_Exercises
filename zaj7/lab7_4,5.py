...
def compile_shaders():
    vertex_shader_source = """
        ...
        void main(void) {
            // Zastosowanie przeksztłacenia w osi X oraz Y
            int moveX  = gl_InstanceID % 10;
            int moveY = int(gl_InstanceID / 10); 
            gl_Position = P_matrix * V_matrix * M_matrix * 
                ( position + (moveX * vec4(1, 0, 0, 0))
                           + (moveY * vec4(0, 1, 0, 0))
                );
...
def render(time):
    ...
    # Oddalenie kamery w celu zobaczenia efektu
    M_matrix = glm.translate(M_matrix, glm.vec3(-4.5, -4.5, 0.0))
    glUniformMatrix4fv(M_location, 1, GL_FALSE, glm.value_ptr(M_matrix))

    # Narysowanie 100 kopii obiektu - z wykorzystaniem rysowania instancjowego
