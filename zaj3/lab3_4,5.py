...
def countViewer0(theta, phi, R):
    value = R * math.cos(theta) * math.cos(phi)
    return value

def render(time):
    global theta
    global phi
    global isCameraModeOn
    global viewer

    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
    glLoadIdentity()

    if not isCameraModeOn:
    ...
    if isCameraModeOn:
        ...
        _phi = phi*(math.pi/180)
        _theta = theta*(math.pi/180) 
        viewer[0] = R * math.cos(_theta) * math.cos(_phi)
        viewer[1] = R * math.sin(_phi)
        viewer[2] = R * math.sin(_theta) * math.cos(_phi)
        gluLookAt(viewer[0], viewer[1], viewer[2],
